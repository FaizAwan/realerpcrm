    ...
    const handleOpenModal = (bill?: any) => {
        if (bill) {
            ...
            setFormData({
                ...
                billNumber: `BILL-${Math.floor(Math.random() * 90000) + 10000}`,
                poNumber: "",
                amount: "",
                status: "unpaid",
                tag: "Material",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                alert("Protocol sequence rejected by central command.");
            }
        } catch (error) {
            alert("Protocol Handshake Failed.");
        }
    };
    ...
    return (
        <div className="space-y-10 pb-4 text-slate-800">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                    ...
                    <p className="text-slate-500 mt-2 font-medium">
                        Streamline Billing, <span className="text-primary font-bold">Accelerate Payments</span>
                    </p>
                </motion.div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                    { label: "Total Receivables", val: bills.length, icon: BarChart3, color: "text-indigo-500" },
                    { label: "Pending Protocol", val: bills.filter(b => b.status === 'unpaid').length, icon: Clock, color: "text-amber-500" },
                    { label: "Settled Ledger", val: bills.filter(b => b.status === 'paid').length, icon: CreditCard, color: "text-green-500" },
                    { label: "Overdue Alerts", val: "00", icon: AlertCircle, color: "text-rose-500" },
                ].map((stat, i) => (
            ...
            <div className="bg-white rounded-xl border border-slate-100 shadow-xl overflow-hidden">
                <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
                    <h4 className="text-xl font-semibold text-slate-800 tracking-tight uppercase">Ledger Stream</h4>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-slate-50/50">
                            <tr>
                                <th className="px-8 py-5 text-[10px] font-semibold text-slate-400 uppercase">Serial</th>
                                <th className="px-8 py-5 text-[10px] font-semibold text-slate-400 uppercase">Vendor</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {isLoading ? (
                                <tr><td colSpan={5} className="py-20 text-center animate-pulse text-slate-300 font-bold uppercase text-xs">Syncing Ledger...</td></tr>
                                ...
                                filteredBills.map((bill) => (
                                    <tr key={bill.id} className="hover:bg-slate-50/50 transition-colors group">
                                        <td className="px-8 py-6 text-sm font-bold text-slate-800 tracking-tight">#{bill.billNumber}</td>
                                        <td className="px-8 py-6 text-sm font-semibold text-slate-600">
                                            {suppliers.find(s => s.id === bill.supplierId)?.name || `ID: ${bill.supplierId}`}
                                        </td>
                                        <td className="px-8 py-6 text-sm font-bold text-slate-800">PKR {bill.amount}</td>
                                        ...
                                        <td className="px-8 py-6 text-right text-slate-800">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => handleOpenModal(bill)} className="p-2 bg-slate-50 text-slate-400 rounded-lg hover:bg-primary hover:text-white transition-all"><Edit2 className="w-4 h-4" /></button>
                                                <button onClick={() => handleDelete(bill.id)} className="p-2 bg-red-50 text-red-400 rounded-lg hover:bg-red-500 hover:text-white transition-all"><Trash2 className="w-4 h-4" /></button>
                                            </div>
                                        </td>
                                    </tr>
                                ))
                ...
                <div className="p-8 bg-slate-50/30 border-t border-slate-50 flex justify-between items-center text-slate-800">
                    <button className="flex items-center gap-2 text-primary font-semibold text-[10px] uppercase tracking-wide hover:underline ">
                        <Zap className="w-4 h-4" />
                        View Detailed Audit Log
                    </button>
                    <p className="text-[10px] font-bold text-slate-400 uppercase">Universal Ledger Interface 2.1</p>
                </div>
            </div>
                        ...
                        <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} className="fixed top-0 right-0 z-[210] h-full w-full max-w-lg bg-white shadow-2xl overflow-y-auto">
                            <div className="h-full flex flex-col">
                                <div className="bg-primary px-4 py-3 text-white flex justify-between items-center shrink-0">
                                    <div>
                                        <h2 className="text-xl font-semibold tracking-normal">{editingBill ? "Refine Protocol" : "Instate Protocol"}</h2>
                                        <p className="text-white/60 text-[10px] font-bold uppercase tracking-wide">Authorized Invoice Data Entry</p>
                                    </div>
                                ...
                                <form onSubmit={handleSubmit} className="p-4 space-y-4 flex-1 flex flex-col justify-center">
                                    <div className="space-y-4">
                                        <div>
                                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-tight pl-1 mb-1 block">Bill Number</label>
                                            <div className="relative group">
                                                ...
                                                <input required className="w-full pl-10 pr-4 py-1.5 bg-slate-50 border border-slate-100 rounded-xl text-sm font-semibold text-slate-800 outline-none focus:ring-4 focus:ring-primary/10 transition-all font-sans" value={formData.billNumber} onChange={e => setFormData({ ...formData, billNumber: e.target.value })} />
                                            </div>
                                        </div>
                                        <div>
                                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-tight pl-1 mb-1 block">Supplier</label>
                                            <div className="relative group">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-tight pl-1 mb-1 block">Amount (PKR)</label>
                                                <input type="number" required className="w-full px-4 py-1.5 bg-slate-50 border border-slate-100 rounded-xl text-sm font-semibold text-slate-800 outline-none focus:ring-4 focus:ring-primary/10 transition-all font-sans" value={formData.amount} onChange={e => setFormData({ ...formData, amount: e.target.value })} />
                                            </div>
                                            <div>
                                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-tight pl-1 mb-1 block">Due Date</label>
                                                <input type="date" required className="w-full px-4 py-1.5 bg-slate-50 border border-slate-100 rounded-xl text-sm font-semibold text-slate-800 outline-none focus:ring-4 focus:ring-primary/10 transition-all" value={formData.dueDate} onChange={e => setFormData({ ...formData, dueDate: e.target.value })} />
                                            </div>
                                        </div>
                                        <div>
                                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-tight pl-1 mb-1 block">Status</label>
                                    ...
                                    <button type="submit" className="w-full py-2 mt-auto bg-primary text-white rounded-xl font-bold text-xs uppercase shadow-lg shadow-primary/20 hover:scale-[1.02] flex items-center justify-center gap-2 transition-all">
                                        Authorize Creation <ChevronRight className="w-4 h-4" />
                                    </button>
                                </form>
                            </div>
                        </motion.div>
