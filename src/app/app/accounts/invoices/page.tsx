...
import { Plus, Filter, Download, Search, MoreVertical, FileText, Calendar, DollarSign, Clock, CheckCircle2, AlertCircle, Loader2 } from "lucide-react";
...
export default function InvoicesPage() {
    ...
    const stats = {
        ...
        paidThisMonth: invoices.filter(i => i.status === 'paid').reduce((sum, i) => sum + Number(i.amount), 0),
        drafts: invoices.filter(i => i.status === 'draft').reduce((sum, i) => sum + Number(i.amount), 0),
    };
        ...
        <div className="space-y-6 max-w-[1400px] mx-auto pb-10">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex gap-2">
                    ...
                    <Link href="/app/accounts/invoices/new" className="flex items-center gap-2 px-4 py-2 bg-[#00605A] text-white rounded-lg text-sm font-medium hover:bg-[#004f4a] transition-colors shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                    { title: "Total Outstanding", value: `Rs ${stats.totalOutstanding.toLocaleString()}`, icon: DollarSign, color: "text-blue-600" },
                    { title: "Overdue", value: `Rs ${stats.overdue.toLocaleString()}`, icon: AlertCircle, color: "text-rose-600" },
                    { title: "Paid This Month", value: `Rs ${stats.paidThisMonth.toLocaleString()}`, icon: CheckCircle2, color: "text-[#059669]" },
                    { title: "Drafts", value: `Rs ${stats.drafts.toLocaleString()}`, icon: Clock, color: "text-slate-500" },
                ].map((stat, i) => (
                    <motion.div
                        key={i}
                ...
                <div className="overflow-x-auto">
                    {loading ? (
                                ...
                                {filteredInvoices.map((invoice, i) => (
                                    <tr key={i} className="hover:bg-slate-50/80 transition-colors group">
                                        <td className="px-6 py-4">
                                            <div className="font-medium text-[#059669] hover:underline cursor-pointer">{invoice.invoiceNumber}</div>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex justify-center">
                                                <span className={`px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${invoice.status === 'paid' ? 'bg-emerald-100 text-emerald-700' :
                        ...
                        <div className="flex flex-col items-center justify-center py-20 gap-2 text-slate-400">
                            ...
                            <button onClick={fetchInvoices} className="text-[#059669] text-sm hover:underline font-bold">Refresh list</button>
                        </div>
                    )}
                </div>
            </div>
